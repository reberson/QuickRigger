...
def create_lattice_nasolabial():
    reset_controls()
    # declare projection groups
    ...
    for side in sides:
        ...
        for rib_point in rib_point_list:
            ...
            # TODO: replace point constraint to parent with shortest interp. But make it positional only (no rotation)
            cmds.pointConstraint(ctrl, mediator)
            follicle = cmds.createNode("follicle")
            follicle_transform = cmds.rename(cmds.listRelatives(follicle, p=True),
                                             "follicle_plane_" + rib_point)
...
def attach_nasolabial_lattice():
    reset_controls()
    const_proj_grp = cmds.group(em=True, n="nasolabial_projection_constraint")
    cmds.parent(const_proj_grp, "face_system")
    rib_jnts = ["NasolabialUpper_R", "NasolabialUpper_L", "NasolabialLower_R", "NasolabialLower_L", "NasolabialMid_R",
                "NasolabialMid_L"]
    for jnt in rib_jnts:
...
def detach_nasolabial_lattice():
    reset_controls()
    # delete lattice constraints
    cmds.delete("nasolabial_projection_constraint")
    cmds.select(d=True)
